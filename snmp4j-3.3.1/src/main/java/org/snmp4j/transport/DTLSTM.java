...
/**
 * The <code>DTLSTM</code> implements the Datagram Transport Layer Security Transport Mapping (TLS-TM) as defined by RFC
 ...
 * <code>listen</code> method is called, or when an outgoing request is sent using the <code>sendMessage</code> method.
 * </p>
 *
 * @author Frank Fock
 * @version 3.0.3
...
public class DTLSTM extends DefaultUdpTransportMapping implements X509TlsTransportMappingConfig,
     ...
     * @since 3.2.0
     */
    public DTLSTM(UdpAddress address, boolean serverEnabled) throws IOException {
        super(address);
     ...
     */
    public DTLSTM(TlsTmSecurityCallback<X509Certificate> securityCallback,
                  UdpAddress serverAddress, CounterSupport counterSupport, boolean serverEnabled) throws IOException {
    ...
    /**
     * Returns the <code>MessageLengthDecoder</code> used by this transport mapping.
     *
...
//    @Override
    ...
    /**
     * Sets the <code>MessageLengthDecoder</code> that decodes the total message length from the header of a message.
     *
    ...
    protected class TlsTrustManager implements X509TrustManager {
        ...
        public void checkClientTrusted(X509Certificate[] x509Certificates, String s) throws CertificateException {
            ...
            if (!useClientMode && (callback != null)) {
                boolean isClientCertificateAccepted = callback.isClientCertificateAccepted(x509Certificates[0]);
                if (isClientCertificateAccepted) {
                    if (logger.isInfoEnabled()) {
                        logger.info("Client is trusted with certificate '" + x509Certificates[0] + "'");
                    }
                    return;
                } else {
                    logger.debug("DTLSTM security callback not configured");
                }
            }
            try {
                trustManager.checkClientTrusted(x509Certificates, s);
            } catch (CertificateException cex) {
                logRejectedCertificate(SnmpConstants.snmpTlstmSessionInvalidClientCertificates,
                        "Client certificate validation failed for '", x509Certificates[0]);
                throw cex;
            }
        }

        public void logRejectedCertificate(OID snmpTlstmSessionInvalidClientCertificates, String s2, X509Certificate x509Certificate) {
            counterSupport.fireIncrementCounter(new CounterEvent(this, SnmpConstants.snmpTlstmSessionOpenErrors));
            counterSupport.fireIncrementCounter(new CounterEvent(this, snmpTlstmSessionInvalidClientCertificates));
            logger.warn(s2 + x509Certificate + "'");
        }
        ...
        public void checkServerTrusted(X509Certificate[] x509Certificates, String s) throws CertificateException {
            ...
            if (entry != null) {
                ...
                String rejectionTxt = "Peer hostname " + hostName + " did not match dNSName " + dNSName;
                logRejectedCertificate(SnmpConstants.snmpTlstmSessionInvalidServerCertificates, rejectionTxt,
                        x509Certificates[0]);
                throw new CertificateException(rejectionTxt);
            }
            try {
                trustManager.checkServerTrusted(x509Certificates, s);
            } catch (CertificateException cex) {
                logRejectedCertificate(SnmpConstants.snmpTlstmSessionInvalidServerCertificates, "Server certificate validation failed for '", x509Certificates[0]);
                throw cex;
            }
            TlsTmSecurityCallback<X509Certificate> callback = securityCallback;
            if (useClientMode && (callback != null)) {
        ...
        private boolean isMatchingFingerprint(X509Certificate[] x509Certificates, OctetString fingerprint) throws CertificateException {
            return TLSTMUtil.isMatchingFingerprint(x509Certificates, fingerprint, false, counterSupport, logger, this);
        }

        @Override
        public X509Certificate[] getAcceptedIssuers() {
            TlsTmSecurityCallback<X509Certificate> callback = securityCallback;
            return org.snmp4j.transport.tls.TlsTrustManager.getAcceptedIssuers(trustManager, callback);
        }
    }
