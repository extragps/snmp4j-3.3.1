...
public class Snmp implements Session, CommandResponder {
     ...
     * <p>
     * To initialize a <code>Snmp</code> instance created with this constructor follow this sample code:
     * <pre>
     * Transport transport = ...;
     ...
     *         TransportMapping the initial <code>TransportMapping</code>. You can add more or remove the same later.
     */
    ...
    /**
     * Creates a <code>Snmp</code> instance by supplying a <code> MessageDispatcher</code> and a
     ...
     *         a <code>MessageDispatcher</code> instance that will be used to dispatch incoming and outgoing messages.
     * @param transportMapping
     *         the initial <code>TransportMapping</code>, which may be <code>null</code>. You can add or remove
     *         transport mappings later using {@link #addTransportMapping} and {@link #removeTransportMapping}
     *         respectively.
     */
    ...
    /**
     * Creates a <code>Snmp</code> instance by supplying a <code> MessageDispatcher</code>.
     * <p>
     ...
     * specified listening address and registers the provided <code>CommandResponder</code> with the internal
     * <code>NotificationDispatcher</code>.
     *
     * @param transportMapping
     *         the TransportMapping that is listening on the provided listenAddress. Call
     *         <code>TransportMappings.getInstance().createTransportMapping(listenAddress);</code> to create such a
     *         transport mapping.
     * @param listenAddress
     *         the <code>Address</code> denoting the transport end-point (interface and port) to listen for incoming
     *         notifications.
     * @param listener
     *         the <code>CommandResponder</code> instance that should handle the received notifications.
     *
     * @return <code>true</code> if registration was successful and <code>false</code>
     * if, for example, the transport mapping for the listen address could not be created.
     * @since 2.5.0
     */
     ...
     *         the listen address.
     *
     * @return the {@link TransportMapping} for the specified listen address or <code>null</code> if there is no
     * notification listener for that address.
     * @since 2.5.0
     */
     ...
     *
     * @throws IOException
     *         if a transport mapping throws an <code>IOException</code> when its {@link TransportMapping#listen()}
     ...
     * called with a <code>null</code> response and a {@link InterruptedException} in the error member of the {@link
     ...
     *
     * @param pdu
     *         a <code>PDU</code> instance. For SNMPv3 messages, the supplied PDU instance has to be a
     *         <code>ScopedPDU</code> instance.
     * @param target
     *         the Target instance representing the target SNMP engine where to send the <code>pdu</code>.
     * @param <A> type of the target {@link Address}
     *
     * @return the received response encapsulated in a <code>ResponseEvent</code> instance. To obtain the received
     * response <code>PDU</code> call {@link ResponseEvent#getResponse()}. If the request timed out, that method will
    ...
    /**
     * Asynchronously sends a GET request <code>PDU</code> to the given target. The response is then returned by calling
     * the supplied
     ...
     *         the Target instance representing the target SNMP engine where to send the <code>pdu</code>.
     * @param userHandle
     *         an user defined handle that is returned when the request is returned via the <code>listener</code>
     *         object.
     * @param listener
     ...
     *
     * @param pdu
     *         a {@code PDU} instance. For SNMPv3 messages, the supplied PDU instance has to be a
     *         {@code ScopedPDU} instance.
     * @param target
     *         the Target instance representing the target SNMP engine where to send the <code>pdu</code>.
     * @param <A> type of the target {@link Address}
     *
    ...
    /**
     * Asynchronously sends a GETNEXT request <code>PDU</code> to the given target. The response is then returned by
     * calling the supplied
     * <code>ResponseListener</code> instance.
     *
     * @param pdu
     *         the PDU instance to send.
    ...
    /**
     * Asynchronously sends a GETBULK request <code>PDU</code> to the given target. The response is then returned by
     * calling the supplied
     * <code>ResponseListener</code> instance.
     *
     * @param pdu
    ...
    /**
     * Asynchronously sends an INFORM request <code>PDU</code> to the given target. The response is then returned by
     * calling the supplied
     * <code>ResponseListener</code> instance.
     *
     * @param pdu
     ...
     * @param target
     *         the Target instance representing the target SNMP engine where to send the <code>pdu</code>. The selected
     *         SNMP protocol version for the target must be {@link SnmpConstants#version1}.
     *
     * @throws IOException
     ...
     * @param target
     *         the Target instance representing the target SNMP engine where to send the <code>pdu</code>. The selected
     *         SNMP protocol version for the target must be {@link SnmpConstants#version2c} or {@link
    ...
    /**
     * Asynchronously sends a SET request <code>PDU</code> to the given target. The response is then returned by calling
     * the supplied
     * <code>ResponseListener</code> instance.
     *
     * @param pdu
    ...
    /**
     * Sends a <code>PDU</code> to the given target and if the <code>PDU</code> is a confirmed request, then the
     * received response is returned synchronously.
     *
     * @param pdu
     *         a <code>PDU</code> instance. When sending a SNMPv1 trap PDU, the supplied PDU instance must be a
     *         <code>PDUv1</code>. For all types of SNMPv3 messages, the supplied PDU instance has to be a
     *         <code>ScopedPDU</code> instance.
     * @param target
     *         the Target instance representing the target SNMP engine where to send the <code>pdu</code>.
     * @param transport
     *         specifies the <code>TransportMapping</code> to be used when sending the PDU. If <code>transport</code> is
     *         <code>null</code>, the associated message dispatcher will try to determine the transport mapping by the
     *         <code>target</code>'s address.
     * @param <A> type of the target {@link Address}
     *
     * @return the received response encapsulated in a <code>ResponseEvent</code> instance. To obtain the received
     * response <code>PDU</code> call {@link ResponseEvent#getResponse()}. If the request timed out, that method will
     * return <code>null</code>. If the sent <code>pdu</code> is an unconfirmed PDU (notification, response, or report),
     * then
     ...
     * @param transport
     *         the (optional) transport mapping to be used to send the request. If <code>transport</code> is
     *         <code>null</code> a suitable transport mapping is determined from the <code>target</code> address.
     ...
     *
     * @return a byte array containing the authoritative engine ID or <code>null</code> if it could not be discovered.
     ...
     *
     * @return MessageProcessingModel a <code>MessageProcessingModel</code> if
     ...
     *
     * @param event
     *         a <code>CommandResponderEvent</code> with the decoded incoming PDU as dispatched to this method call by
     *         the associated message dispatcher.
    ...
    /**
     * Checks whether RFC5343 based context engine ID discovery is disabled or not. The default value is
     * <code>false</code>.
     *
    ...
    /**
     * Sets the RFC5343 based context engine ID discovery. The default value is <code>false</code>.
     *
    ...
    /**
     * Adds a <code>CommandResponder</code> to this SNMP session. The command responder will then be informed about
     * incoming SNMP PDUs of any kind that are not related to any outstanding requests of this SNMP session.
     *
     * @param listener
    ...
    /**
     * Fires a <code>CommandResponderEvent</code> event to inform listeners about a received PDU. If a listener has
     * marked the event as processed further listeners will not be informed about the event.
     *
     * @param event
     ...
     * by the <code>timeout</code> parameter of the target has elapsed without a response being received for the
     * request. By specifying a different timeout model this behaviour can be changed.
     *
     * @param timeoutModel
    ...
    protected class PendingRequest<A extends Address> extends TimerTask implements PduHandleCallback<PDU>, Cloneable {
        ...
        /**
         * The <code>nextPDU</code> field holds a PDU that has to be sent when the response of the <code>pdu</code> has
         * been received. Usually, this is used for (context) engine ID discovery.
         */
        private PDU nextPDU;
        ...
        /**
         * Cancels the request and clears all internal fields by setting them to <code>null</code>.
         *
    ...
    /**
     * The <code>NotificationDispatcher</code> dispatches traps, notifications, and to registered listeners.
     *
     * @author Frank Fock
     * @version 2.5.0
     * @since 1.6
