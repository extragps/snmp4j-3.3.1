...
/**
 * A <code>CommunityTarget</code> represents SNMP target properties for community based message processing models
 * (SNMPv1 and SNMPv2c).
 *
 * @author Frank Fock
 * @version 3.1
...
public class CommunityTarget<A extends Address> extends AbstractTarget<A> {
     ...
     *
     * @return an <code>OctetString</code> instance, should be never <code>null</code> when using this target to send
     ...
     * (v1 and v2c). It basically checks that the community is not <code>null</code> and then calls {@link
